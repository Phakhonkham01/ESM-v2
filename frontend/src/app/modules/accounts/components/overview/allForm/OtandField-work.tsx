...
interface OTFieldWorkProps {
  onClose: () => void
  onSuccess?: () => void
  type: 'OT' | 'FIELD_WORK' // เพิ่ม prop type
}

const OtandFieldWork: React.FC<OTFieldWorkProps> = ({ onClose, onSuccess, type }) => {
    ...
    const loadSupervisors = async () => {
      try {
        ...
        const supervisorUsers = (response.data || []).filter(
          (user: User) => user.role === 'supervisor' && user.status === 'Active'
        )
  ...
  try {
    ...
    // ส่ง supervisor_ids ทั้งหมดแทนที่จะเป็นคนเดียว
    const supervisorIds = matchingSupervisors.map(s => s.id || s.id)

    const requestData = {
      user_id: currentUser._id,
      supervisor_id: supervisorIds, // ส่งเป็น array
      date: formData.date,
    ...
    // alert(`${config.title} submitted successfully to ${supervisorIds.length} supervisor(s)!`)
    onSuccess?.()
    onClose()
  } catch (error: any) {
  ...
  className='form-control form-control-lg'
  rows={4}
  placeholder={type === 'OT' ? 'Describe the reason for overtime' : 'Describe the purpose and location of field work'}
  value={formData.reason}
  onChange={(e) => setFormData({ ...formData, reason: e.target.value })}
  required  // เพิ่มบรรทัดนี้
/>
          </div>
        </div>
