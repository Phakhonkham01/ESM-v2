...
const UsersListFilter = () => {
  ...
  const resetData = () => {
    ...
    // Reset to initial state with default year
    updateState({ 
  ...
  const filterData = () => {
    ...
    // Always include year (either selected or default)
    if (year) filter.year = year

    if (month) filter.month = month
    
    // Only add department if it's selected and not the default option
    if (department && department !== '' && department !== 'All Departments') {
      filter.department = department
    }
    
    // Only add status if it's selected and not the default option
    ...
    <>
      ...
      <div className='menu menu-sub menu-sub-dropdown w-350px w-md-400px' data-kt-menu='true'>
        ...
        <div className='px-7 py-5'>
          <div className='fs-5 text-gray-900 fw-bolder'>Filter OT/Field Work Requests</div>
        </div>
